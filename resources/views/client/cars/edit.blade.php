    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-3 mb-8">
                <a href="{{ route('client.cars.show', $car) }}" class="p-2 rounded-xl hover:bg-gray-100 transition text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </a>
                <h1 class="text-2xl font-bold text-gray-900">Editează — {{ $car->brand }} {{ $car->model }}</h1>
            </div>

            <div class="bg-white rounded-2xl border border-gray-100 p-8">
                <form method="POST" action="{{ route('client.cars.update', $car) }}">
                    @csrf @method('PUT')

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label for="brand" class="block text-sm font-medium text-gray-700 mb-1.5">Marca</label>
                            <input id="brand" name="brand" type="text" value="{{ old('brand', $car->brand) }}" required
                        ...
                        <div>
                            ...
                            <input id="model" name="model" type="text" value="{{ old('model', $car->model) }}" required
                                   class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition" />
                        ...
                        <div>
                            ...
                            <input id="year" name="year" type="number" value="{{ old('year', $car->year) }}" required min="1900" max="{{ date('Y') + 1 }}"
                                   class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition" />
                        ...
                        <div>
                            ...
                            <input id="plate" name="plate" type="text" value="{{ old('plate', $car->plate) }}" required
                                   class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition" />
                    ...
                    <div class="flex items-center justify-end mt-8 gap-3">
                        <a href="{{ route('client.cars.show', $car) }}" class="px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-900 rounded-xl transition">Anulează</a>
                        <button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 transition shadow-sm">Salvează modificările</button>
                    </div>
                </form>
            </div>
        </div>
